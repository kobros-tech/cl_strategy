...
class SkillMemoryStrategy:
    ...
    def train(self, experience) -> None:
        ...
        After Skill Memory training completes, the strategy automatically:

        1. records the class-to-experience mapping;
        2. consolidates all retained evaluation memory;
        3. trains the auxiliary ML evaluator on accumulated memory;
        4. records class-level loss and accuracy;
        5. computes the current diagonal metrics;
        6. optionally evaluates the actual stored Skill Memory.
        """
        ...
        # --------------------------------------------------------------
        # 2. Optional direct Skill Memory evaluation
        # --------------------------------------------------------------

        self._evaluate_skill_memory(experience_index)

        # --------------------------------------------------------------
        # 3. Build accumulated evaluator memory
        # --------------------------------------------------------------
        ...
        if not accumulated_memory:
            raise RuntimeError(
                "Evaluation memory is empty after Skill Memory training."
            )

        # --------------------------------------------------------------
        # 4. Train auxiliary ML evaluator
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 5. Evaluate auxiliary ML evaluator
        # --------------------------------------------------------------
        ...
        missing_classes = [
            class_id for class_id in classes if class_id not in current_accuracy
        ]

        if missing_classes:
            raise RuntimeError(
                f"Evaluator returned no results for classes {missing_classes}."
            )

        diagonal_accuracy = float(
            np.mean([current_accuracy[class_id] for class_id in classes])
        )

        diagonal_loss = float(np.mean([current_loss[class_id] for class_id in classes]))
        ...
        self._last_evaluation = {
            ...
            "experience_losses": losses,
            "experience_accuracies": accuracies,
    ...
    def evaluate(self) -> dict[str, Any]:
        """Return the latest evaluator results."""
        if self._last_evaluation is None:
            raise RuntimeError(
                "No evaluation is available. Train at least one experience."
            )
    ...
    def results(self) -> dict[str, Any]:
        ...
        - final class loss
        - diagonal accuracy/loss history
        - acquisition-relative forgetting
        - peak-relative forgetting
        - mean final accuracy/loss
        - mean forgetting
        - direct Skill Memory metrics when enabled
        """
        ...
        result: dict[str, Any] = {
            ...
            "mean_diagonal_accuracy": float(np.mean(self._diagonal_accuracy_history)),
            ...
            "mean_class_forgetting_peak_relative": float(peak_forgetting.mean()),
        }
